...
output_path = 'difference_of_differences_results.csv'  # Replace with your desired output path
...
for country in countries:
    ...
    for var in education_vars:
        ...
        for area in ['rural', 'urban']:
            ...
            chart = alt.Chart(country_df).mark_point().encode(
                x=alt.X('year:O', axis=alt.Axis(title='Year')),  # Year as ordinal
                y=alt.Y(f'{area_var}:Q',
                        axis=alt.Axis(title=f'{custom_titles[var]} ({area.capitalize()})'),
                        scale=alt.Scale(zero=False)),
                tooltip=['year', area_var]
            ).properties(
                width=360,
                height=360,
                title=f"{custom_titles[var]} in {country}'s {area.capitalize()} Areas"
            )
